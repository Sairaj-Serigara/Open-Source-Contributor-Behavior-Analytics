...
repo_status = pd.crosstab(df["repository"], df["status"])

plt.figure(figsize=(7, 5))

repo_status.plot(kind="bar", stacked=True)
...
plt.hist(df["contributions"], bins=20)
...
plt.scatter(df["contributions"], df["days_since_last_commit"])
...
plt.hist(df["contributions"], bins=20, edgecolor="black")
...
plt.hist(df["days_since_last_commit"], bins=20, edgecolor="black")
